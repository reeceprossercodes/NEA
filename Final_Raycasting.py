...
class Ray:
    ...
    def cast(self):
        ...
        self.distance *= math.cos(self.player.rotationAngle - self.angle)   #fisheye correction elminates the curviness of the lines when player is close to the wall
...
class Raycasting:
    ...
    def __init__(self, game, player, map, renderer):
        # Name: __init__
    ...
    def render(self, screen):
        ...
        for dist, image, pos in sorted(self.objects_to_render, key=lambda x: -x[0]):
        ...
        #         distance = Distance_inbetween(self.player.x, npc.x, self.player.y, npc.y)
        #         visible_npcs.append((distance, npc, angle_diff))
        #
        # # Sort by distance (farthest first)
        # visible_npcs.sort(reverse=True)
        #
        # for distance, npc, angle_diff in visible_npcs:
        ...
        #     ray_index = int((angle_diff / FOV) * NumRays)
        #     ray_index = max(0, min(ray_index, NumRays - 1))
        #     if distance < self.rays[ray_index].distance:
        #         # NPC is visible
